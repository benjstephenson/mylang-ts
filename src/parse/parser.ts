...
import { Expr, Identifier, InfixExpr, LetDeclaration, NumericLiteral, ObjectLiteral, Program, Property } from "./ast.ts"
import { tokenise } from "../lex/lexer.ts"
import { IdentifierToken, Token, TokenType, isCloseBraceToken, isColonToken, isEofToken, isEqualsToken, isIdentifierToken, isInfixOperatorToken, isLetToken, isOpenBraceToken } from "../lex/Tokens.ts"
...
function buildExprParser(operators: InfixOperator[], parser: (tokens: Token[]) => [Token[], Expr]): (tokens: Token[]) => [Token[], Expr] {

  const _parse = (tokens: Token[], expr: Expr,): [Token[], Expr] => {

    const [head, ...tail] = tokens

    if (!isInfixOperatorToken(head) || !operators.includes(head.value)) return [tokens, expr]
...
const parseMultiplicativeExpr = buildExprParser([Symbols.Slash, Symbols.Asterisk, Symbols.Percent], parseExponentialExpr)
const parseAdditiveExpr = buildExprParser([Symbols.Plus, Symbols.Minus], parseMultiplicativeExpr)
...
function parseObjectExpr(tokens: Token[]): [Token[], Expr] {
  ...
  const _parseObjectExpr = (tokens: Token[], props: Property[]): [Token[], Property[]] => {
    const [head, t, ...tail] = tokens

    if (!head || isCloseBraceToken(head) || isEofToken(head)) return [[t!, ...tail], props]

    if (!t) throw new Error(`Uneven object literal, ${isIdentifierToken(head) ? `${head.value} must have a value` : `no named value to assign`}`)

    if (isIdentifierToken(head)) {
      const [remainingTokens, expr] = parseExpr([t, ...tail])
      return _parseObjectExpr(remainingTokens, A.push(Property(head.value, expr), props))
    }
    ...
    if (isColonToken(head)) {
      ...
      throw new Error(`Cannot assign a ${t._tag} expression to a naked identifier in object literal`)
    }
